...
object HBaseSource {
  ...
  def main(args: Array[String]) {
    val spark = SparkSession.builder()
      .appName("HBaseSourceExample").master("local")
      .getOrCreate()
    ...
    def withCatalog(cat: String): DataFrame = {
      sqlContext
        .read
        .options(Map(HBaseTableCatalog.tableCatalog->cat, HBaseRelation.HBASE_CONFIGURATION -> s"""{"hbase.zookeeper.quorum":"emr-header-1,emr-header-2,emr-worker-1"}"""))
    ...
    sc.parallelize(data).toDF.write.options(
      Map(HBaseTableCatalog.tableCatalog -> cat, HBaseTableCatalog.newTable -> "5", HBaseRelation.HBASE_CONFIGURATION -> s"""{"hbase.zookeeper.quorum":"emr-header-1,emr-header-2,emr-worker-1"}"""))
      .format("org.apache.spark.sql.execution.datasources.hbase")
      .save()

    // for testing connection sharing only
    sc.parallelize(data).toDF.write.options(
      Map(HBaseTableCatalog.tableCatalog -> cat1, HBaseTableCatalog.newTable -> "5", HBaseRelation.HBASE_CONFIGURATION -> s"""{"hbase.zookeeper.quorum":"emr-header-1,emr-header-2,emr-worker-1"}"""))
      .format("org.apache.spark.sql.execution.datasources.hbase")
      .save()
